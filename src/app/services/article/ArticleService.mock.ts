...
import HtmlParser from 'app/parser/html/HtmlParser.mock';
...
export function getArticle(url : ArticleURL) : Promise <Article> {
	...
	let fileExtension;
	switch (url.href) {
		case 'http://test/xyz/1':
			path = 'resources/article/article-get-01.json';
			break;
		case 'http://test/xyz/0':
			path = 'resources/article/article-get.json';
			break;
		case 'http://test/xyz/page/0':
			path = 'resources/page/page-get.html';
			break;
		case 'http://test/xyz/page/1':
			path = 'resources/page/page-get.html';
			break;
		default:
			path = 'resources/article/article-get-01.json';
	...
	if (fileExtension) {
		if (fileExtension === '.json') {
			return <Promise<Article>> app.loadJSON(path);
		} else if (fileExtension === '.html') {
			return <Promise<Article>> new HtmlParser(path)
				.getArticle()
	...
	throw new TypeError(`Unexpected file extension: ${fileExtension}`);
}

...
class UsersGriddle extends React.Component <IUsersGriddle, any> {
	...
	render() {
		...
		const ActionBar = ({ value, griddleKey, rowData }) => (
			<div>
				<button onClick={this.onEdit} data-item={JSON.stringify(rowData)} className="button success" type="button">Edit</button>
				<button onClick={this.onDestroy} data-id={rowData.id} className="button alert" type="button">Delete</button>
			</div>);
			...
			<section>
				<Griddle data={usersGrid}>
					<RowDefinition>
						...
						<ColumnDefinition id="actions" title="Actions" customComponent={enhancedWithRowData(ActionBar)} />
					</RowDefinition>
				</Griddle>
			</section>
		);
